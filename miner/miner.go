		...
		******************************************************************************************************************************************************/
		...
		if responses, err := pipeliner.Pipelined(reqCtx, func(pipeliner redis.Pipeliner) error {
			...
			for idT0, amount := range balanceT1EthereumIncr {
				...
				if err := pipeliner.HIncrByFloat(reqCtx, model.SerializedUsersKey(idT0), "balance_t1_tmp2_ethereum_pending", amount).Err(); err != nil {
					return err
				}
			}
			for idTMinus1, amount := range balanceT2EthereumIncr {
				...
				if err := pipeliner.HIncrByFloat(reqCtx, model.SerializedUsersKey(idTMinus1), "balance_t2_tmp2_ethereum_pending", amount).Err(); err != nil {
					return err
				}
			}
