...
func (c *config) globalAggregationIntervalChildDateFormat() string {
	...
	switch c.GlobalAggregationInterval.Child { //nolint:exhaustive // We only worry about those.
...
func (c *config) globalAggregationIntervalParentDateFormat() string {
	const hoursInADay = 24
	switch c.GlobalAggregationInterval.Parent { //nolint:exhaustive // We only worry about those.
	case stdlibtime.Minute:
		return minuteFormat
	case stdlibtime.Hour:
		return hourFormat
...
func (c *config) lastXMiningSessionsCollectingIntervalDateFormat() string {
	const hoursInADay = 24
	switch c.RollbackNegativeMining.LastXMiningSessionsCollectingInterval { //nolint:exhaustive // We only worry about those.
	case stdlibtime.Minute:
		return minuteFormat
	case stdlibtime.Hour:
		return hourFormat
